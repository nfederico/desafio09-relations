...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    if (findProductsWithNoQtyAvailable.length) {
      throw new AppError(
        `The qty  ${findProductsWithNoQtyAvailable[0].quantity} is not available for ${findProductsWithNoQtyAvailable[0].id}`,
      );
    }
    ...
    const orderedProductsQty = order_products.map(product => ({
